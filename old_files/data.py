...
import sys
#import operator
#from iprocessor import process_image
...
#        label_hot = np_utils.to_categorical(label_encoded, len(self.classes))
        label_hot = np.eye(len(self.classes), dtype='float32')[label_encoded]        
#        label_hot = label_hot[0]  # just get a single row
    ...
    def get_all_sequences_in_memory(self, batch_Size, train_test, data_type, concat=False):
        ...
        y1 = np.expand_dims(yn,0)
        y2 = y1.repeat(self.seq_length,0)
...
#        path = sequence_path + filename + '-' + str(40) + \
#           '-' + data_type + '.npy' 
