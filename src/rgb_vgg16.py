...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 14:20:25 2019
...
__all__ = ['VGG', 'rgb_vgg16_pretrained', 'rgb_vgg16_bn']
...
import torch
def rgb_vgg16_pretrained(num_classes, pretrained=True, **kwargs):
...
def main():
    num_classes = 20
    model = rgb_vgg16_pretrained(num_classes, pretrained= True)
    x = np.random.rand(224,224,3)
    x = np.reshape(x, (1,224,224,3))
    x = np.transpose(x, (0,3,1,2))
    x = torch.tensor(x).float()
    g = model(x) 
    print('Output shape in VGG16: ', g.shape)
