...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 24 16:51:09 2019
...
#           'rgb_resnet152', 'rgb_resnet200']

__all__ = ['ResNet', 'rgb_resnet200', 'rgb_resnet200_pretrained']
...
model_urls = {
    ...
    'resnet200': 'https://download.pytorch.org/models/resnet200-37304a01b.pth',
}
...
def rgb_resnet200(pretrained=False, **kwargs):
    model = ResNet(Bottleneck, [3, 24, 36, 3], **kwargs)
    return model

def rgb_resnet200_pretrained(num_classes, pretrained= True, **kwargs):
    """Constructs a ResNet-200 model.
    Args:
        pretrained (bool): If True, returns a model pre-trained on ImageNet
    """
    model = ResNet(Bottleneck, [3, 24, 36, 3], num_classes, **kwargs)
    if pretrained:
        # model.load_state_dict(model_zoo.load_url(model_urls['resnet152']))
#        pretrained_dict = model_zoo.load_url(model_urls['resnet152'])
        pretrained_dict = torch.load('/home/rajs/.torch/models/resnet_200-37304a01b.pth')
        model_dict = model.state_dict()
...
def main():
    num_classes = 1000
    model = rgb_resnet200_pretrained(num_classes)
    m = 224
    n = 224
    x = np.random.rand(m,n,3)
    x = np.reshape(x, (1,m,n,3))
    x = np.transpose(x, (0,3,1,2))
    x = torch.tensor(x).float()
    g = model(x) 
    print('Output shape in Resnet: ', g.shape)
